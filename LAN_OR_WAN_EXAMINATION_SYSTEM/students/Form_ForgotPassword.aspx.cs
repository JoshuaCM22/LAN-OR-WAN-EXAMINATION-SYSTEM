...
namespace LAN_OR_WAN_EXAMINATION_SYSTEM.students // Created by: Joshua C. Magoliman
{
    public partial class Form_ForgotPassword : System.Web.UI.Page
    {
        ...
        private void Checking()
        {
            ...
            {
                Objects.con.Open();
                Objects.cmd = new SqlCommand("SELECT * FROM tbl_students WHERE email=@email AND secret_question=@secret_question", Objects.con);
                ...
                if (Objects.dr.Read() && Objects.dr.GetValue(0) != DBNull.Value)
                {
                    ...
                    if (isMatch == true)
                    {
                        Form_SetANewPassword.studentId = Convert.ToInt32(Objects.dr["id"].ToString());
                        Objects.con.Close();
                        Objects.dr.Close();
                        Response.Redirect("../students/Form_SetANewPassword.aspx");
                    }
                    else
                    {
                        Objects.dr.Close();
